...
for _, group in tqdm(groups, desc="Calculating Batting Average", unit="player"):
    ...
    cumulative_boundaries = (
        (group["fours"] + group["sixes"]).cumsum().shift(fill_value=0)
    )
    ...
    dismissals = (
        ~group["dismissal_info"].str.contains("not out", case=False, na=False)
    ).cumsum()
    dismissals.replace(0, 1, inplace=True)

    data.loc[group.index, "batting_avg"] = cumulative_runs / dismissals
    data.loc[group.index, "strike_rate_over_time"] = (
        cumulative_runs / cumulative_balls
    ) * 100
    data.loc[group.index, "boundary_per_ball"] = (
        cumulative_boundaries / cumulative_balls
    )
    data.loc[group.index, "boundary_idx"] = cumulative_boundaries / innings
    data.loc[group.index, "finishing_idx"] = np.where(
        innings > 0, dismissals / innings, 0
    )
    data.loc[group.index, "runs_without_boundary"] = (
        cumulative_runs - cumulative_boundaries
    ) / innings
    data.loc[group.index, "big_impact_idx"] = (
        (2 * cumulative_100) + cumulative_50
    ) / innings
