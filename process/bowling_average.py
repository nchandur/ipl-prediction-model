...
for _, group in tqdm(groups, desc="Calculating Bowling Average", unit="player"):
    ...
    data.loc[group.index, "econ_over_time"] = (
        cumulative_runs_conceded / cumulative_overs
    )
    data.loc[group.index, "balls_per_innings"] = cumulative_balls / innings
    data.loc[group.index, "wickets_idx"] = np.where(
        innings > 0, cumulative_wickets / innings, 0
    )
    ...
    data.loc[group.index, "strike_rate_over_time"] = (
        cumulative_balls / cumulative_wickets
    )
